...
window.StudentDelete = {
    ...
    setupListeners() {
        document.getElementById('btnDeleteSelection').onclick = () => {
        ...
        document.getElementById('cancelSelectionBtn').onclick = () => this.exitDeleteMode();
        document.getElementById('confirmDeleteSelectionBtn').onclick = () => this.confirmDeleteSelected();
    },

    async deleteStudent(studentId) {
        const confirmed = await window.confirm('هل أنت متأكد من إخلاء مربع هذا الطالب؟ سيبقى رقمه التسلسلي محفوظاً.');
        if (!confirmed) return;
        ...
        students = students.map(student =>
            String(student.id) === String(studentId)
                ? window.StudentStore.clearStudentDetails(student)
                : student
        );
    ...
    confirmDeleteSelected() {
        ...
        if (!confirm(`هل أنت متأكد من إخلاء مربعات ${this.parent.selectedIds.size} طلاب؟`)) return;

        let students = window.StudentStore.getStudents(this.parent.currentStage, this.parent.currentGrade);
        students = students.map(student =>
            this.parent.selectedIds.has(student.id)
                ? window.StudentStore.clearStudentDetails(student)
                : student
        );
    ...
    openDeleteAllModal() {
        ...
        confirmBtn.onclick = () => {
            window.StudentStore.clearStudents(
