...
window.StudentDelete = {
    ...
    async confirmDeleteSelected() {
        ...
        if (!confirm(`هل أنت متأكد من حذف بيانات ${this.parent.selectedIds.size} طلاب نهائياً؟`)) return;
        ...
        await Promise.all(students
            .filter(student => this.parent.selectedIds.has(student.id) && !window.StudentStore.isEmptySlot(student))
            .map(student => window.StudentStore.hardDeleteStudentData(
                this.parent.currentStage,
                this.parent.currentGrade,
                student
            )));
