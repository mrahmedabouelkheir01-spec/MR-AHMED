...
window.StudentStore = {
    ...
    hardDeleteStudentData(stage, grade, studentRecord) {
        ...
        this.clearCache();
        window.AppwriteConfig?.deleteStudentData?.(studentRecord.id).catch(error => {
    ...
    purgeExpiredExpelledStudents(stage = null, grade = null) {
        ...
        contexts.forEach(context => {
            ...
            const remaining = records.filter(record => {
                ...
                if (expired) {
                    removedCount += 1;
                    this.hardDeleteStudentData(context.stage, context.grade, record);
                }
                return !expired;
            });
    ...
    clearStudents(stage, grade) {
        ...
        window.AppwriteConfig?.deleteStudentsInGrade?.(stage, grade).catch(error => {
