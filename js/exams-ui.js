window.ExamsUI = {
    ...
    finishExam(examIdx) {
        ...
        const confirmed = confirm('هل أنت متأكد من إنهاء هذا الامتحان؟ سيتم تسجيل جميع الطلاب بدون درجات كـ (غائب/راسب)');
        if (!confirmed) return;
    ...
    deleteColumn(columnIdx) {
        if (!confirm('هل أنت متأكد من حذف هذا العمود بالكامل؟')) return;
